...
def first_species(cf, prev, mode):
	...
	for option in options:
		...
		ends_unison = not(len(cf) == 2 and not(utils.scale_degree(option[1], mode) == 2))
		recover_leap = True
		...
		if len(prev) > 1:
			...
			recover_leap = not(len(prev) > 1 and abs(prev_intrvl) > 4 and (prev_intrvl*intrvl > 0 or abs(intrvl) > 2))
		perfect_row = not (utils.is_perfect(prev_h[1]) and prev_h[1] == option[1])
		not_same_note = not option[0] == prev_h[0]
		no_p_ints_similar = not (utils.is_perfect(option[1]) and intrvl*(prev_h[0] - prev_h[1] - note) < 0)
